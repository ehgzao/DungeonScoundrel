...
if (btnWelcomeMusicToggle && typeof music !== 'undefined') {
    btnWelcomeMusicToggle.onclick = () => {
        if (music.isPlaying) {
            music.stop();
            btnWelcomeMusicToggle.textContent = '‚?∂Ô∏è';
        } else {
            music.start();
...
if (btnMusicToggle && typeof music !== 'undefined') {
    btnMusicToggle.onclick = () => {
        if (music.isPlaying) {
            music.stop();
            btnMusicToggle.textContent = '‚?∂Ô∏è';
        } else {
            music.start();
...
btnStartGameModal.onclick = () => {
    ...
    if (name.length < 3) {
        nameError.textContent = '‚ù? Name must have at least 3 characters!';
    ...
    if (name.length > 10) {
        nameError.textContent = '‚ù? Name must have maximum 10 characters!';
        nameError.style.display = 'block';
        playerNameInput.style.borderColor = '#ff6b6b';
        playerNameInput.focus();
        return;
...
const CLASSES = {
    scoundrel: {
        ...
        mechanics: '<strong>‚??Ô∏è Passive:</strong> None - Pure skill and luck<br><br><strong>‚?® Active:</strong> None - Just you and your wits<br><br><em style="color: #8b7355; font-size: 0.9em;">This is the baseline class. Master the basics before seeking power.</em>',
        icon: '??≠',
        unlocked: true,
    ...
    knight: {
        ...
        mechanics: '<strong>??°Ô∏è Passive:</strong> +5 Max HP | Weapons have +1 durability<br><br><strong>‚??Ô∏è Active (Shield Bash):</strong> Deal damage equal to your weapon value to the first monster in the room. Cooldown: 3 rooms.',
        icon: '??°Ô∏è',
        unlocked: false,
    ...
    rogue: {
        ...
        mechanics: '<strong>??°Ô∏è Passive:</strong> Can hold 2 cards instead of 1 | Start with 1 extra gold per room<br><br><strong>??™ Active (Shadow Strike):</strong> Next monster takes double weapon damage and doesn\'t break combo. Cooldown: 4 rooms.',
        icon: '??°Ô∏è',
        unlocked: false,
    ...
    berserker: {
        ...
        mechanics: '<strong>??¢ Passive (Bloodlust):</strong> Damage increases as HP decreases<br>‚?¢ +1 damage at ‚?§70% HP<br>‚?¢ +2 damage at ‚?§50% HP<br>‚?¢ +3 damage at ‚?§30% HP<br><br><strong>‚??Ô∏è Active (Rage Strike):</strong> Sacrifice 5 HP for triple damage (3x) on next 3 attacks. Breaks combo. Cooldown: 4 rooms.<br><em style="color: #ff6b6b; font-size: 0.9em;">‚?†Ô∏è Cannot use if HP ‚?§ 5</em>',
        icon: '??¢',
        unlocked: false,
    ...
    priest: {
        ...
        mechanics: '<strong>???Ô∏è Passive (Divine Blessing):</strong> 15% chance to negate damage completely | Potions heal +2 HP | Start with +2 Max HP<br><br><strong>??ø Active (Purification):</strong> Permanently remove the strongest monster from current dungeon OR transform a monster into a potion. Cooldown: 6 rooms.<br><em style="color: #ffd700; font-size: 0.9em;">‚?® Strategic: Eliminate threats before facing them</em>',
        icon: '??ø',
        unlocked: false,
...
if (btnTopGiveUp && giveUpModal && btnCancelGiveUp && btnConfirmGiveUp) {
    btnTopGiveUp.addEventListener('click', function() {
        if (game.gameOver || game.gameStartTime === 0) {
            showMessage('‚?†Ô∏è Start a game first!', 'warning');
            return;
        }
    ...
    console.error('‚ù? Give Up elements not found:', {
...
// Special Cards Types
const specialCards = [
    { id: 'dodge', name: '??°Ô∏è Dodge', description: 'Avoid next damage', effect: () => { game.dodgeActive = true; showMessage('??°Ô∏è Dodge active!', 'success'); createParticles(window.innerWidth / 2, window.innerHeight / 2, '#4ecdc4', 20); } },
    ...
    { id: 'lucky_draw', name: '??≤ Lucky Draw', description: 'Draw 3 cards with favorable odds', effect: () => {
        ...
        showMessage(`??≤ Lucky Draw! Drew ${drawnCards.length} cards (${weaponCount}‚??Ô∏è ${potionCount}‚ù§Ô∏è)`, 'success');
        earnGold(5);
...
function useClassAbility() {
    ...
    if (!game.classData.active) {
        showMessage('‚ù? Scoundrel has no special abilities!', 'warning');
        playSound('error');
        return;
    }
...
function useKnightAbility() {
    ...
    if (!game.equippedWeapon) {
        showMessage('‚?†Ô∏è Need a weapon equipped to use Shield Bash!', 'warning');
        playSound('error');
        return;
    }
    ...
    if (!firstMonster) {
        showMessage('‚?†Ô∏è No monsters in room!', 'warning');
        playSound('error');
        return;
    }
    ...
    if (firstMonster.numValue <= 0) {
        ...
        showMessage(`??°Ô∏è Shield Bash! Monster defeated!`, 'success');
    } else {
        showMessage(`??°Ô∏è Shield Bash! Dealt ${damage} damage! (${firstMonster.numValue} HP left)`, 'success');
    }
...
function useBerserkerAbility() {
    ...
    if (game.health <= HEALTH.CRITICAL_THRESHOLD) {
        showMessage(`‚?†Ô∏è Not enough HP! Need more than ${HEALTH.CRITICAL_THRESHOLD} HP to use Rage Strike.`, 'danger');
        playSound('error');
        return;
    }
    ...
    if (game.combo > 0) {
        showMessage(`??¢ Rage Strike! -5 HP, next attack: 3x damage! ‚?†Ô∏è Combo broken! (Lost ${game.combo}x)`, 'warning');
        game.combo = 0;
    } else {
...
function usePriestAbility() {
    ...
    if (monsters.length === 0) {
        showMessage('‚?†Ô∏è No monsters in dungeon to purify!', 'warning');
        playSound('error');
        return;
    }
    ...
    if (choice) {
        // Remove permanently
        game.dungeon.splice(index, 1);
        showMessage(`??ø Purification! ${strongestMonster.value}${strongestMonster.suit} removed from existence!`, 'success');
    } else {
        ...
        showMessage(`??ø Purification! ${strongestMonster.value}${strongestMonster.suit} transformed into ${potionValue}‚?• potion!`, 'success');
    }
...
function generateTooltip(card) {
    ...
    if (card.isBoss) {
        ...
        return `<span class="tooltip-negative">??π <strong>${bossName}</strong> - ${card.numValue}/${card.maxHP || card.numValue} HP<br>${card.bossFlavor || 'Defeat this boss to continue!'}</span>`;
    }
    ...
    if (type === 'monster') {
        ...
        if (baseWeapon === 0) {
            return `<span class="tooltip-negative">‚?†Ô∏è No weapon! Damage: -${card.numValue} HP</span>`;
        }
        ...
        if (game.rageStrikeActive && game.classAbilityCounter > 0) buffInfo += `‚??Ô∏è3x `;
        
        if (game.dodgeActive) return `<span class="tooltip-positive">??°Ô∏è Dodge: No damage! ${buffInfo}</span>`;
        ...
        if (actualDamage > 0) return `<span class="tooltip-negative">‚??Ô∏è Fight: -${actualDamage} HP ${buffInfo}</span>`;
        else return `<span class="tooltip-positive">‚??Ô∏è Perfect! No damage! ${buffInfo}</span>`;
        
    } else if (type === 'weapon') {
        const current = (game.equippedWeapon?.numValue || 0) + powerBonus;
        const cardValue = card.numValue + powerBonus;

        if (cardValue > current) return `<span class="tooltip-positive">‚¨?Ô∏è Better Weapon! (${current} ‚?? ${cardValue})</span>`;
        else if (cardValue < current) return `<span class="tooltip-negative">‚¨?Ô∏è Worse Weapon (${current} ‚?? ${cardValue})</span>`;
        else return `<span class="tooltip-neutral">‚??Ô∏è Same Weapon (${cardValue})</span>`;
        
    } else if (type === 'potion') {
        const maxPotions = (game.classData && game.classData.passive.maxPotionsPerRoom) || 1;
        if (game.potionsUsed >= maxPotions) {
            return `<span class="tooltip-negative">‚ù? Potion limit reached (${maxPotions}/${maxPotions})</span>`;
        }
    ...
    if (game.playerClass) {
        ...
        const passiveIcons = {
            ...
            berserker: [
                ...
                { icon: '‚??Ô∏è', text: 'High Risk', title: '+1/+2/+3 damage at ‚?§70%/50%/30% HP' }
            ],
            priest: [
                { icon: '???Ô∏è', text: '15% Dodge', title: '15% chance to dodge damage' },
...
const IN_GAME_TUTORIAL_STEPS = [
    ...
    {
        id: 'cards',
        title: '??è Understanding Cards',
        text: '‚?†Ô∏è‚?£Ô∏è = Monsters (damage you)\n‚?¶Ô∏è = Weapons (equip to fight)\n‚?•Ô∏è = Potions (heal you)\n‚?® = Special (powerful effects)',
        highlight: '#room',
        position: 'top',
    ...
    if (step.highlight) {
        ...
        if (targetElement) {
            ...
            if (rect.width > 0 && rect.height > 0) {
                ...
                console.warn('[TUTORIAL] ‚?†Ô∏è Element not visible (0 dimensions):', step.highlight);
            }
        } else {
            console.warn('[TUTORIAL] ‚?†Ô∏è Element not found:', step.highlight);
        }
    }
    ...
    if (skipBtn) {
        skipBtn.onclick = () => {
            ...
            confirmModal.innerHTML = `
                <h2 style="color: #ff4444; margin-bottom: 20px; font-size: 1.6em;">‚?†Ô∏è Skip Tutorial?</h2>
...
function drawRoom() {
    if (game.dungeon.length === 0) {
        ...
        if (game.difficulty === 'endless') {
            ...
            showMessage(`‚?æÔ∏è ENDLESS MODE: Deck ${game.endlessLevel} loaded! Monsters +${Math.floor(difficultyScaling * 100)}% HP!`, 'warning');
            playSound('special');
    ...
    if (nextRoomNumber === 14) {
        showMessage('‚?†Ô∏è MINIBOSS APPROACHING! Prepare for a tough fight!', 'warning');
    } else if (nextRoomNumber === 24) {
        showMessage('‚?†Ô∏è FINAL MINIBOSS APPROACHING! Get ready!', 'warning');
    }
    ...
    if (isMiniboss1 || isMiniboss2) {
        ...
        playSound('special');
        addLog(`‚?†Ô∏è MINIBOSS #${minibossNumber} BATTLE! ${boss.name} has ${bossHP} HP!`, 'danger');
        showMessage(`??π MINIBOSS: ${boss.name}`, 'danger');
    ...
    if (game.relics.some(r => r.id === 'tank')) {
        game.mirrorShield = 1;
        showMessage('?è∞ Fortress Armor: +1 HP shield', 'success');
    }
...
function avoidRoom() {
    ...
    if (game.lastActionWasAvoid && !hasClover) {
        showMessage('‚ù? You cannot avoid 2 dungeons in a row!', 'warning');
        playSound('error');
        return;
    }
...
function undoLastMove() {
    if (!game.undoAvailable || !game.lastGameState) {
        showMessage('‚ù? No move to undo!', 'warning');
        return;
    }
    ...
    showMessage('‚?©Ô∏è Move undone!', 'info');
    playSound('cardFlip');
    updateUI();
}
...
function handleMonster(monster, index) {
    ...
    if (monster.isBoss) {
        ...
        if (!game.equippedWeapon) {
            ...
            showMessage(`??π ${monster.bossName || 'Boss'} attacked and fled! -${bossDamage} HP (NO REWARD!)`, 'danger');
            playSound('damage');
            screenShake();
            ...
            setTimeout(() => {
                showMessage(`‚?†Ô∏è No weapon equipped = No gold! Always bring a weapon to boss fights!`, 'warning');
            }, 1500);
        ...
        if (monster.numValue <= 0) {
            ...
            const victoryMessages = [
                '‚??Ô∏è The Knight falls silent. The curse is broken...',
            ...
            showMessage(`??π ${monster.bossName} DEFEATED! +${bossGold} GOLD!`, 'success');
            ...
            showMessage(`‚??Ô∏è Hit boss for ${effectiveWeapon} damage! Boss HP: ${monster.numValue}`, 'info');
            playSound('attack');
            updateUI();
    ...
    if (permanentUnlocks.thornsArmor && damage > 0) {
        monster.numValue -= 2;
        showMessage('??µ Thorns Armor reflected 2 damage!', 'info');
        ...
        if (monster.numValue <= 0) {
            ...
            showMessage('??µ Thorns Armor killed the monster (after taking damage)!', 'warning');
            resetCombo();
            updateUI();
            checkGameState();
            return;
    ...
    if (game.relics.some(r => r.id === 'weak_thorns') && damage > 0) {
        monster.numValue -= 1;
        showMessage('??ø Weak Thorns reflected 1 damage!', 'info');
        ...
        if (monster.numValue <= 0) {
            ...
            showMessage('??ø Weak Thorns killed the monster (after taking damage)!', 'warning');
            resetCombo();
            updateUI();
            checkGameState();
            return;
    ...
    if (game.relics.some(r => r.id === 'armor') && damage > 0) {
        ...
        if (damage < originalDamage) {
            if (damage === 0) {
                showMessage(`?¶æ Iron Armor absorbed all ${originalDamage} damage!`, 'success');
            } else {
                showMessage(`?¶æ Iron Armor reduced damage! (${originalDamage} ‚?? ${damage})`, 'info');
            }
        }
    }
    ...
    if (cloakRelic && damage > 0) {
        ...
        damage = 0;
        showMessage('?ß• Cloak blocked damage!', 'success');
        weaponWasUsed = false;
        attackWasMade = false;
        game.combo++;
        game.stats.maxCombo = Math.max(game.stats.maxCombo, game.combo);
    ...
    if (game.dodgeActive && damage > 0) {
        ...
        showMessage('??°Ô∏è Dodged! No damage!', 'success');
    }
    // Priest Divine Blessing - 15% chance to dodge
        ...
        showMessage('???Ô∏è Divine Blessing! No damage!', 'success');
    ...
    // Perfect Kill (ONLY if weapon is equipped!)
        ...
        if (lifesteal > 0) {
            ...
            showMessage(`?ß? +${lifesteal} HP from Vampirism!`, 'success');
        }
        ...
        if (criticalHit) {
            ...
            showMessage(`‚??Ô∏è Perfect kill! ${game.combo}x COMBO!`, 'success');
        }
    }
    // TAKE DAMAGE: Monster hits you
        ...
        if (!game.equippedWeapon) {
            ...
            showMessage(`‚?†Ô∏è NO WEAPON! -${damage} HP`, 'danger');
        } else {
    ...
    if (weaponWasUsed && game.equippedWeapon && game.equippedWeapon.durability < 999 && !weaponDurabilityReduced) {
        ...
        if (game.equippedWeapon.durability <= 0) {
            ...
            const emoji = game.equippedWeapon.durability === 1 ? '‚?†Ô∏è' : '‚??Ô∏è';
    ...
    if (monster.isBoss) {
        ...
        const bossGold = bossGoldByDifficulty[game.difficulty] || 20;
        earnGold(bossGold);
        showMessage(`??π BOSS DEFEATED! +${bossGold} gold!`, 'success');
    ...
    if (game.health <= 0) {
        ...
        if (phoenix) {
            phoenix.used = true;
            game.health = 10;
            showMessage('?ê¶ Phoenix Feather activated! Revived with 10 HP!', 'success');
            playSound('special');
            updateRelicsDisplay();
        }
    }
...
function handleWeapon(weapon, index) {
    ...
    showMessage(`‚??Ô∏è Equipped weapon with value ${weapon.numValue + powerBonus}! (${game.equippedWeapon.durability}/${game.equippedWeapon.maxDurability} uses)`, 'success');
...
function showGameOver(title, message, score, scoreLabel, isVictory, gameTime, reason, gaveUp) {
    ...
    if (isVictory) {
        ...
        if (perfectRunBonus > 0) bonusesHTML += `<p style="color: #ffd700;">?è? Perfect Run (No Damage): +${perfectRunBonus}</p>`;
        if (shopPenalty > 0) bonusesHTML += `<p style="color: #ff6b6b;">?è∫ Shop Penalty (${game.stats.shopsVisited || 0} visits): -${shopPenalty}</p>`;
        ...
        scoreBreakdownHTML = `
            <div class="game-over-stats" style="background: rgba(0,0,0,0.4); border: 1px solid #ffd700; margin-bottom: 15px; text-align: left;">
                ...
                <p>‚??Ô∏è Monster Bonus (${game.stats.monstersSlain}): +${monsterBonus}</p>
                ${bonusesHTML}
    ...
    if (isVictory) {
        ...
        (async () => {
            ...
            try {
                ...
                btn.textContent = '‚ù? Submission Failed';
                ...
                btn.onclick = async () => {
                    ...
                    try {
                        ...
                        setButtonLoading(btn, false);
                        btn.textContent = '‚ù? Failed Again';
...
function updateUI() {
    ...
    if (game.health > 0 && game.health <= HEALTH.CRITICAL_THRESHOLD) {
        ...
        if (!game.criticalWarningShown) {
            showMessage('‚?†Ô∏è CRITICAL HP! Find healing soon!', 'danger');
    ...
    if (hasMap && game.dungeon.length > 0) {
        ...
        dungeonCountEl.textContent = `${game.dungeon.length} (??∫Ô∏è ${preview})`;
    } else if (hasCandle && game.dungeon.length > 0) {
        const preview = game.dungeon[0];
        dungeonCountEl.textContent = `${game.dungeon.length} (??ØÔ∏è ${preview.value}${preview.suit})`;
    } else {
    ...
    if (game.equippedWeapon) {
        ...
        if (game.equippedWeapon.durability < 999) {
            ...
            const durabilityIcon = game.equippedWeapon.durability === 1 ? '‚?†Ô∏è' : '‚??Ô∏è';
...
function createCardElement(card) {
    ...
    if (type === 'special') {
        ...
        cardEl.innerHTML = `
            <div class="card-value" style="font-size: 2em;">??π</div>
        ...
        if (type === 'monster' && card.numValue > 0) {
            ...
            if (totalDamage === 0) {
                ...
                dmgBadge.textContent = `‚??Ô∏è ${totalDamage}`;
                dmgBadge.style.background = 'linear-gradient(135deg,#6bcf7f,#2fbf71)';
            } else {
                dmgBadge.textContent = `‚??Ô∏è ${totalDamage} (-${netDamage})`;
